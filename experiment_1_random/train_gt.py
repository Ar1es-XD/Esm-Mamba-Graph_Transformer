# -*- coding: utf-8 -*-
"""
train_gt.py - Experiment 1: Random Split
End-to-end MambaCross Graph Transformer network trainer for Experiment 1 (Random Split).
"""
import os
import sys
import json
...
def load_split():
    """Load train/test pairs for Random Split partitioning (prefer local folder CSVs)."""
    ...
    if os.path.exists(train_csv) and os.path.exists(test_csv):
        ...
        print(f"[Exp 1 - Random Split] Loaded directly from local folder: {len(train_pairs)} Train, {len(test_pairs)} Test pairs.")
    ...
    print(f"[Exp 1 - Random Split] Loaded from ab_ag_pair.csv: {len(train_pairs)} Train, {len(test_pairs)} Test pairs.")
    return train_pairs, test_pairs
...
def main(epochs=30, batch_size=32, sampling=True, sample_fraction=0.15):
    ...
    print(f"[Exp 1 - Random Split] Filtered: {len(train_filtered)} Train, {len(test_filtered)} Test pairs.")
    ...
    if sampling:
        ...
        print(f"[Exp 1 - Random Split] Sampled (fraction={sample_fraction}): {len(train_filtered)} Train, {len(test_filtered)} Test pairs.")
    ...
    if os.path.exists(checkpoint_path):
        try:
            ...
            print(f"-> Resuming Exp 1 from checkpoint: epoch {start_epoch} (best AUC so far: {best_auc:.4f})")
        except Exception as e:
            print(f"Could not load checkpoint: {e}. Starting from scratch.")

    print(f"Training Exp 1 (Random Split) on {device} from epoch {start_epoch+1} to {epochs}..." if start_epoch > 0
          else f"Training Exp 1 (Random Split) on {device} for {epochs} epochs...")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        print(f"  [Exp 1] Epoch {epoch+1:>2}/{epochs} Loss={avg_loss:.4f} AUC={auc_score:.4f} AUPRC={aupr_score:.4f}")
    ...
    print(f"[Success] [Exp 1] Best AUC={best_auc:.4f} (epoch {best_metrics.get('Best Epoch')}). Saved -> {results_json}")
    return best_metrics


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train ESM-Mamba Graph Transformer on Experiment 1 (Random Split)")
