# -*- coding: utf-8 -*-
"""
train_gt.py - Experiment 3: Novel Antibodies
End-to-end MambaCross Graph Transformer network trainer for Experiment 3 (Novel Antibodies).
"""
import os
import sys
import json
...
def load_split():
    """Load train/test pairs for Novel Antibodies partitioning (prefer local folder CSVs)."""
...
def main(epochs=30, batch_size=32, sampling=True, sample_fraction=0.15):
    ...
    if sampling:
        ...
        print(f"[Exp 3 - Novel Antibodies] Sampled (fraction={sample_fraction}): {len(train_filtered)} Train, {len(test_filtered)} Test pairs.")
    ...
    if os.path.exists(checkpoint_path):
        try:
            ...
            print(f"-> Resuming Exp 3 from checkpoint: epoch {start_epoch} (best AUC so far: {best_auc:.4f})")
        except Exception as e:
            print(f"Could not load checkpoint: {e}. Starting from scratch.")

    print(f"Training Exp 3 (Novel Antibodies) on {device} from epoch {start_epoch+1} to {epochs}..." if start_epoch > 0
          else f"Training Exp 3 (Novel Antibodies) on {device} for {epochs} epochs...")
    ...
    print(f"[Success] [Exp 3] Best AUC={best_auc:.4f} (epoch {best_metrics.get('Best Epoch')}). Saved -> {results_json}")
    return best_metrics


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train ESM-Mamba Graph Transformer on Experiment 3 (Novel Antibodies)")
